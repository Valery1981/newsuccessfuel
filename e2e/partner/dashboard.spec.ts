...
test.describe("Partner Dashboard — KPIs et Tableaux", () => {
  test.skip(!TEST_CREDS.partenaire.email, "TEST_PARTENAIRE_EMAIL non configuré");

  test.beforeEach(async ({ page }) => {
    await loginAs(page, TEST_CREDS.partenaire.email, TEST_CREDS.partenaire.password);
  ...
  test("KPIs cards visibles", async ({ page }) => {
    await expect(
      page.getByText(/achat carburant/i).first()
    ).toBeVisible({ timeout: 10_000 });
    await expect(
      page.getByText(/vente carburant/i).first()
    ).toBeVisible();
    await expect(
      page.getByText(/achat lubrifiants/i).first()
    ).toBeVisible();
    await expect(
      page.getByText(/doléances ouvertes/i).first()
    ).toBeVisible();
  });

  test("tableau niveau de stock visible", async ({ page }) => {
    await expect(
      page.getByText(/niveau de stock par station/i)
    ).toBeVisible({ timeout: 10_000 });
  });
