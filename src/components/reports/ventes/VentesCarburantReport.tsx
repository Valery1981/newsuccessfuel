...
export function VentesCarburantReport() {
  const { entreprise } = useAuthStore();
  const [filters, setFilters] = useState<ReportFilterValues>(defaultFilterValues());
  const { data: stations = [] } = useReportStations();

  const { data: rows = [], isLoading } = useQuery<ShiftRow[]>({
    queryKey: ["report-ventes-carburant", filters, entreprise?.id],
    queryFn: async () => {
      ...
      let query = supabase
        ...
        .select("id, date_shift, station_id, ca_total, statut, pompiste_id, tiers(nom)")
  ...
  const totalCA = rows.reduce((acc, r) => acc + r.ca_total, 0);
  ...
  function handleExport() {
    exportCsv(rows.map(r => ({
      ...
      "CA (Ar)": r.ca_total,
      Statut: r.statut,
    })), `ventes-carburant-${filters.dateDebut}-${filters.dateFin}`);
  }

  return (
    <ReportLayout title="Ventes carburant" description="Shifts carburant par station et par pompiste" onExport={handleExport}>
          ...
          <div className="rounded-md border overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {rows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center text-muted-foreground py-10">Aucune vente sur cette période</TableCell>
                  </TableRow>
                ) : rows.map(r => (
                  <TableRow key={r.id}>
