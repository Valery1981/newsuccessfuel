...
export function StockCarburantReport() {
  ...
  const { data: rows = [], isLoading } = useQuery<CuveStock[]>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("cuves")
        .select(
          "id, nom, type_carburant, station_id, stock_actuel_litres, capacite_max, cmup",
        )
      ...
      return (data ?? []).map((c) => ({
        ...
        station_nom:
          stations.find(
            (s) =>
              s.id === ((c as Record<string, unknown>).station_id as string),
          )?.nom ?? "—",
        volume_actuel:
          ((c as Record<string, unknown>).stock_actuel_litres as number) ?? 0,
        capacite: ((c as Record<string, unknown>).capacite_max as number) ?? 0,
        prix_achat_moyen: ((c as Record<string, unknown>).cmup as number) ?? 0,
        valeur_stock:
          (((c as Record<string, unknown>).stock_actuel_litres as number) ??
            0) * (((c as Record<string, unknown>).cmup as number) ?? 0),
      }));
    },
  ...
  function handleExport() {
    exportCsv(
      rows.map((r) => ({
        ...
        "Taux remplissage (%)":
          r.capacite > 0 ? Math.round((r.volume_actuel / r.capacite) * 100) : 0,
  ...
  return (
    <ReportLayout
      ...
      description="État des stocks dans les cuves à ce jour"
              ...
              <TableBody>
                {rows.length === 0 ? (
                  ...
                  rows.map((r) => {
                    const taux =
                      r.capacite > 0
                        ? Math.round((r.volume_actuel / r.capacite) * 100)
                        : 0;
                    return (
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-medium ${produitColor[r.produit] ?? "bg-gray-100 text-gray-800"}`}
                          >
                            {r.produit}
                          </span>
                        </TableCell>
                        <TableCell className="text-right text-sm">
                          {r.volume_actuel.toLocaleString("fr-FR")}
                        </TableCell>
                        <TableCell className="text-right text-sm text-muted-foreground">
                          {r.capacite.toLocaleString("fr-FR")}
                        </TableCell>
                        ...
                        <TableCell className="text-right text-sm">
                          {r.prix_achat_moyen.toLocaleString("fr-FR")}
                        </TableCell>
                        <TableCell className="text-right text-sm font-medium">
                          {formatCurrency(r.valeur_stock)}
                        </TableCell>
                      </TableRow>
                    );
                  })
